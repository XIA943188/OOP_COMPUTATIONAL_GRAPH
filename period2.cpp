...
int main()
{
	...
	for (int i = 1; i <= n; ++i) {
		...
		if (TempStr == "SIN" || TempStr == "LOG" || TempStr == "EXP" || TempStr == "TANH" ||
			TempStr == "SIGMOID" || TempStr == "GRAD" || TempStr == "ASSERT") { //单目
			string OP;
			cin >> OP;
			...
			if (TempStr == "SIN")
				...
				Sample_Graph.BuildCalcNode<GradCNode<double>>(NodeName, 1, OperandsList);
			else if (TempStr == "ASSERT")
				Sample_Graph.BuildCalcNode<AssertCNode<double>>(NodeName, 1, OperandsList);
		}
		else if (TempStr == "PRINT") { //调试输出
			cin >> TempStr;
			Sample_Graph.BuildPriNode(NodeName, TempStr);
		}
		else if (TempStr == "BIND" || TempStr == "ASSIGN") {
			string OP1, OP2;
			cin >> OP1 >> OP2;
			OperandsList.push_back(OP1); OperandsList.push_back(OP2);
			if (TempStr == "BIND")
				Sample_Graph.BuildCalcNode<BindCNode<double>>(NodeName, 2, OperandsList);
			...
			if (OPRT == "+")
				...
				Sample_Graph.BuildCalcNode<DerCNode<double>>(NodeName, 2, OperandsList);
		}
